- 👋 Hi, I am Mariam Ali, a ***Software Engineering*** graduate from SEECS, NUST
- 👀 I’m interested in Web Development, Data Science and Machine Learning
- 🌱 I’m currently working as Frontend and AI Developer
- 📫 How to reach me ma.mariamali.2000@gmail.com
- 📫 Please visit my [LinkedIn](https://www.linkedin.com/in/mariam-ali-02889423a/)

***Professional Experience***
- AI Developer at Kounteq (Remote, UK) - Aug 2024 to Present
- Frontend Developer at URUSystems (Remote, Ohio) - Jan 2024 to July 2024
- Software Engineer at Devsinc (Onsite, Islamabad) - July 2022 to Dec 2023

***Certifications***
-  Coursera - IBM Data Engineering Essentials
-  Coursera - Python for Data Science, AI, and Development
-  Coursera - Python Project for Data Engineering
-  Coursera - Relational Databases (RDBMS) Essentials
-  Coursera - Databases and SQL for Data Science with Python
-  Coursera - Relational Database Administration
-  Coursera - Hands-on Introduction to Linux commands and shell scripting
  
***Skills***
- Web Development (ROR, Javascript, MERN, REACT, REACT Native, Redux Toolkit, NEXT JS, Stripe, HTML, CSS, Bootstrap, Tailwindcss)
- Machine Learning and Computer Vision (Python, OpenCV, Streamlit, Pandas, NumPy, Seaborn, Matplotlib, LabelImg)
- Large Language Models (LLaVA - 13B, 7B)
- Database Platform: MYSQL, PostgreSQL, Kaggle, Cloudinary, Firebase, WapmServer
- Version Control and Deployment Platform (Github/GitLab, Heroku, Netlify, Nginx)
- Project Management Tools (JIRA, Trello, Shortcut)
- Apprehension of Design and Algorithms
- Technical Writing Skills
- Problem Solving and ResearchAnalysis
- Communication Skills (French Language Level-01)
- IELTS band 08



